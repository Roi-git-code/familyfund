...
const regex = {
  ...
  middleName: /^[A-Za-z]{1}$/,     // allow single letter middle name
  email: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
...
function validateName(input, isMiddle = false) {
  ...
  if (isMiddle && regex.middleName.test(value)) return setFeedback(input, "✓ Middle name accepted.", true);
  if (!regex.name.test(value)) return setFeedback(input, "Only letters allowed (max 50).", false);
...
function validateEmail(input) {
  const value = input.value.trim();
  if (!regex.email.test(value)) return setFeedback(input, "Enter a valid email (e.g., name@example.com).", false);
...
function validatePhone(input) {
  const value = input.value.trim();
  if (!regex.phone.test(value)) return setFeedback(input, "Enter valid phone (e.g., +255712345678).", false);
...
function validateAddress(input) {
  const value = input.value.trim();
  if (!regex.address.test(value)) return setFeedback(input, "Address must be P.O.BOX 12345, NAME or S.L.P 12345, NAME.", false);
...
function validateConfirmPassword(password, confirm) {
  if (password.value !== confirm.value) return setFeedback(confirm, "Passwords do not match.", false);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Member forms (application/edit)
  const memberForms = document.querySelectorAll("form[action^='/oma/apply']");
